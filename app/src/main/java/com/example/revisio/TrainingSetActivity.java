...
import com.example.revisio.databinding.ActivityTrainingSetBinding;

import java.util.ArrayList;
public class TrainingSetActivity extends AppCompatActivity {

    public static final String TRAIN = "Training mode";
    private SetRecyclerViewAdapter adapter;
    private DataBaseHelper db;
    private ActivityTrainingSetBinding binding;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = ActivityTrainingSetBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());

        db = new DataBaseHelper(TrainingSetActivity.this);
        ArrayList<SetModel> setModels = db.getSetsModel();

        adapter = new SetRecyclerViewAdapter(this, TRAIN);

        binding.trainingSetsRecyclerView.setLayoutManager(new LinearLayoutManager(this));
        binding.trainingSetsRecyclerView.setAdapter(adapter);
    ...
    public void onBackPressed() {
        Intent intent = new Intent(TrainingSetActivity.this, MainActivity.class);
